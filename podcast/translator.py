...
_TITLE_PROMPT = (
    "你是一个中英文翻译专家。请将以下英文视频标题翻译成中文。\n"
    "必须遵守的规则：\n"
    "1. 必须全部译成中文，不得保留任何英文句子\n"
    "2. 仅保留以下专有名词原样：公司名(Meta/Google/Apple/GitHub/Midjourney)、"
    "产品名(ChatGPT/iPhone/iPad)、缩写(AI/API/GPT/LLM/AGI/Fable/GPU)、人名(PewDiePie/Odysseus/Florence/Alex/Phil)\n"
    "3. 去掉所有标点符号（！？，、；：\"'…()[]{}—–-|）\n"
    "4. 中文和英文之间加一个空格隔开\n"
    "5. 只返回翻译结果，不要任何解释、不要加引号\n"
    "6. 对于带竖线 | 的标题，竖线两边各自翻译\n"
    "重要：必须输出中文，不能输出英文！\n"
    "示例：\n"
    ...
    "  → '重塑熵 压缩即智能 第一部分'\n"
    "  'The first iPod wasn\\'t a success'\n"
    "  → '第一个 iPod 并不是一个成功'\n"
